...
export class ActorSheetSeaCharacter extends ActorSheet {
    ...
    getData() {
        ...
        for (let [key, skill] of Object.entries(data.data.skills)) {
            skill.label = game.i18n.localize(SEA.skills[key]);
        }
    ...
    activateListeners(html) {
        ...
        html.find(".locked").click(ev => {
            const actor = this.actor;
            if ($(ev.currentTarget).attr("data-locked") == 0) {
                actor.update({ "data.locked": 1 });
            } else {
                actor.update({ "data.locked": 0 });
            }
        });
